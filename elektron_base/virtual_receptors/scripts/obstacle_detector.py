...
class ObstacleDetector():
	...
	def __init__(self,name,enable_head,enable_top_kinect,enable_bottom_kinect):
		print "[Obstacle detector] - Virtual receptor <<Cameras>> initialization"
		...
		print "[Obstacle detector] - Waits for clients ..."
	def subscribeToROS(self):
		rospy.Subscriber("/laser_scan", LaserScan, laserCallBack)

	# Checking existance of real effectors
	def checkSubsystems(self):
		print "[Obstacle detector] - Checking existance of real receptors -> not implemented"

	# Setting variables
	def setVariables(self,enable_head,enable_top_kinect,enable_bottom_kinect):
		try:
			print "[Obstacle detector] - Setting variables"
	...
	def openServices(self):
		try:
			print "[Obstacle detector] - setting services"
			print "[Obstacle detector] - service - [captureImage]"
			self.service_map = rospy.Service('rapp_capture_image', GetImage, self.rapp_captrure_image_interface)
		except Exception, ex_map:
			print "[Obstacle detector] - Exception %s" % str(ex_map)
...
def signal_handler(signal, frame):
	print "[Obstacle detector] - signal SIGINT caught"
	print "[Obstacle detector] - system exits"
	sys.exit(0)
...
def main(node_name,enable_head,enable_top_kinect,enable_bottom_kinect):
	...
	try:
		...
		print "[Obstacle detector] - Press Ctrl + C to exit system correctly"
		...
		print "[Obstacle detector] - SystemExit Exception caught"
		#unsubscribe()

		sys.exit(0)
		
	except Exception, ex:
		print "[Obstacle detector] - Exception caught %s" % str(ex)
		#unsubscribe()
		sys.exit(0)
